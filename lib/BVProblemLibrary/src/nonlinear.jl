...
function prob_bvp_nonlinear_1_f!(du, u, p, t)
    du[1] = u[2]
    return du[2] = 1 / p * prob_bvp_nonlinear_1_f(t, u[1], u[2])
end
...
"""
prob_bvp_nonlinear_1 = BVProblem(
    prob_bvp_nonlinear_1_function,
    [1.0, 0.0],
...
function prob_bvp_nonlinear_2_f!(du, u, p, t)
    du[1] = u[2]
    return du[2] = 1 / p * prob_bvp_nonlinear_2_f(u[2])
end
function prob_bvp_nonlinear_2_bca!(res_a, u_a, p)
    return res_a[1] = u_a[1] - 1 - p * log(cosh(-0.745 / p))
end
function prob_bvp_nonlinear_2_bcb!(res_b, u_b, p)
    return res_b[1] = u_b[1] - 1 - p * log(cosh(0.255 / p))
end
...
"""
prob_bvp_nonlinear_2 = BVProblem(
    prob_bvp_nonlinear_2_function,
    [1.0, 0.0],
...
"""
prob_bvp_nonlinear_3 = BVProblem(
    prob_bvp_nonlinear_3_function,
    [1.0, 0.0],
...
"""
prob_bvp_nonlinear_4 = BVProblem(
    prob_bvp_nonlinear_4_function,
    [1.0, 0.0],
...
function prob_bvp_nonlinear_5_f!(du, u, p, t)
    du[1] = u[2]
    return du[2] = prob_bvp_nonlinear_5_f(u[1], p)
end
...
"""
prob_bvp_nonlinear_5 = BVProblem(
    prob_bvp_nonlinear_5_function,
    [1.0, 0.0],
...
function prob_bvp_nonlinear_6_f(t, u1, u2, p)
    return ((1 + γ) / 2 - p * 2 * t) * u1 * u2 - u2 / u1 - 2 * t / A(t) * (1 - (γ - 1) / 2 * u1^2)
end
function prob_bvp_nonlinear_6_f!(du, u, p, t)
    du[1] = u[2]
    return du[2] = prob_bvp_nonlinear_6_f(t, u[1], u[2], p) / (p * A(t) * u[1])
end
function prob_bvp_nonlinear_6_bca!(res_a, u_a, p)
    return res_a[1] = u_a[1] - 0.9129
end
function prob_bvp_nonlinear_6_bcb!(res_b, u_b, p)
    return res_b[1] = u_b[1] - 0.375
end
...
"""
prob_bvp_nonlinear_6 = BVProblem(
    prob_bvp_nonlinear_6_function,
    [1.0, 1.0],
...
function prob_bvp_nonlinear_7_f!(du, u, p, t)
    du[1] = u[2]
    return du[2] = 1 / p * prob_bvp_nonlinear_7_f(u[1], u[2])
end
function prob_bvp_nonlinear_7_bca!(res_a, u_a, p)
    return res_a[1] = u_a[1] + 1 / 3
end
function prob_bvp_nonlinear_7_bcb!(res_b, u_b, p)
    return res_b[1] = u_b[1] - 1 / 3
end
...
"""
prob_bvp_nonlinear_7 = BVProblem(
    prob_bvp_nonlinear_7_function,
    [1.0, 0.0],
...
function prob_bvp_nonlinear_8_f!(du, u, p, t)
    du[1] = u[2]
    return du[2] = 1 / p * prob_bvp_nonlinear_8_f(u[1], u[2])
end
...
"""
prob_bvp_nonlinear_8 = BVProblem(
    prob_bvp_nonlinear_8_function,
    [1.0, 0.0],
...
function prob_bvp_nonlinear_9_f!(du, u, p, t)
    du[1] = u[2]
    return du[2] = 1 / p * prob_bvp_nonlinear_9_f(u[1], u[2])
end
...
[Reference](https://archimede.uniba.it/~bvpsolvers/testsetbvpsolvers/?page_id=252)
"""
prob_bvp_nonlinear_9 = BVProblem(
    prob_bvp_nonlinear_9_function,
    [1.0, 0.0],
...
"""
prob_bvp_nonlinear_10 = BVProblem(
    prob_bvp_nonlinear_10_function,
    [1.0, 0.0],
...
function prob_bvp_nonlinear_11_f!(du, u, p, t)
    du[1] = u[2]
    return du[2] = 1 / p * prob_bvp_nonlinear_11_f(u[1], u[2])
end
...
"""
prob_bvp_nonlinear_11 = BVProblem(
    prob_bvp_nonlinear_11_function,
    [1.0, 0.0],
...
function prob_bvp_nonlinear_12_f!(du, u, p, t)
    du[1] = u[2]
    return du[2] = 1 / p * prob_bvp_nonlinear_12_f(u[1], u[2])
end
function prob_bvp_nonlinear_12_bca!(res_a, u_a, p)
    return res_a[1] = u_a[1] + 7 / 6
end
...
"""
prob_bvp_nonlinear_12 = BVProblem(
    prob_bvp_nonlinear_12_function,
    [1.0, 0.0],
...
"""
prob_bvp_nonlinear_13 = BVProblem(
    prob_bvp_nonlinear_13_function,
    [1.0, 0.0, 0.0, 0.0],
...
function prob_bvp_nonlinear_14_f!(du, u, p, t)
    ...
    du[2] = u[3]
    du[3] = u[4]
    return du[4] = prob_bvp_nonlinear_14_f(u[1], u[2], u[3], u[4], p)
end
function prob_bvp_nonlinear_14_bca!(res_a, u_a, p)
    res_a[1] = u_a[1]
    return res_a[2] = u_a[2]
end
function prob_bvp_nonlinear_14_bcb!(res_b, u_b, p)
    res_b[1] = u_b[1] - 1
    return res_b[2] = u_b[2]
end
...
function prob_bvp_nonlinear_15_f!(du, u, p, t)
    ...
    du[4] = u[5]
    du[5] = u[6]
    return du[6] = 1 / p * (-u[3] * u[6] - u[1] * u[2])
end
function prob_bvp_nonlinear_15_bca!(res_a, u_a, p)
    res_a[1] = u_a[1] + 1
    res_a[2] = u_a[3]
    return res_a[3] = u_a[4]
end
...
"""
prob_bvp_nonlinear_15 = BVProblem(
    prob_bvp_nonlinear_15_function,
